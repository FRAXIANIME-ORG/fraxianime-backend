...
import xyz.kiridepapel.fraxianimebackend.exception.AnimeExceptions.RepeatedChapter;
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(RepeatedChapter.class)
    public ResponseEntity<?> handleRepeatedChapter(RepeatedChapter ex) {
        ResponseDTO response = new ResponseDTO(ex.getMessage(), 404);
        return new ResponseEntity<>(response, HttpStatus.valueOf(response.getStatus()));
    }
