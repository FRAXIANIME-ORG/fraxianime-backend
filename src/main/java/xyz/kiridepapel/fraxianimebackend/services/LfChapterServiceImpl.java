...
import xyz.kiridepapel.fraxianimebackend.interfaces.ILfChapterService;
...
@Service
@Log
public class LfChapterServiceImpl implements ILfChapterService {
  // Variables estaticas
  ...
  // Constructor
  public LfChapterServiceImpl(CacheManager cacheManager, AnimeUtils animeUtils) {
  ...
  @PostConstruct
  private void init() {
    this.ignoreSearchChapterCases = List.of(
      "one-piece", "naruto"
    );
    this.ignoreCases = List.of(
      "part", "season"
    );
  }
  ...
  private ChapterDTO findChapter(String modifiedUrlChapter, String chapter) {
    try {
  ...
  private String calcNextChapterDate(String lastChapterDate, String formatIn, String formatOut, boolean isProduction) {
